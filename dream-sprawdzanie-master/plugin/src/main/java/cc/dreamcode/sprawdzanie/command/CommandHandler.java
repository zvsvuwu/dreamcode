...
import cc.dreamcode.sprawdzanie.command.annotations.RequiredPlayer;
import cc.dreamcode.sprawdzanie.config.MessageConfig;
import cc.dreamcode.sprawdzanie.exception.PluginRuntimeException;
import cc.dreamcode.sprawdzanie.exception.PluginValidatorException;
...
public abstract class CommandHandler extends Command implements PluginIdentifiableCommand, NoticeSender, CommandPlatform {
    ...
    public Plugin getPlugin() {
        return SprawdzaniePlugin.getSprawdzaniePlugin();
    }

    @Override
    public boolean execute(@NonNull CommandSender sender, @NonNull String commandLabel, @NonNull String[] arguments) {
        final MessageConfig messageConfig = SprawdzaniePlugin.getSprawdzaniePlugin().getInject(MessageConfig.class)
    ...
    public @NonNull List<String> tabComplete(@NonNull CommandSender sender, @NonNull String label, @NonNull String[] args) {
        ...
        this.argumentHandlers
                .stream()
                .map(argumentClass -> SprawdzaniePlugin.getSprawdzaniePlugin().createInstance(argumentClass))
